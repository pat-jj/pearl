#!/bin/bash
# monitor_assr.sh — print a single status snapshot for all running ASSR paper jobs.
set -uo pipefail
PROJECT=.

ts=$(date +'%Y-%m-%d %H:%M:%S')
echo "================================================================"
echo "= ASSR paper status @ ${ts}"
echo "================================================================"
...
# ── EM cleanup runs ──
ttail "pure_assr_em (no-warmup cleanup)"    "${PROJECT}/results/pure_assr_em_rerun_paperb.log" 2
ttail "assr_em_rerun (with-warmup cleanup)" "${PROJECT}/results/assr_em_rerun_paper.log" 2

# ── BCOT cleanup runs ──
ttail "pure_assr_bcot (no-warmup cleanup)"    "${PROJECT}/results/pure_assr_bcot_rerun_paperb.log" 3
ttail "assr_bcot_rerun (with-warmup cleanup)" "${PROJECT}/results/assr_bcot_warmup_paperb.log" 3
...
em_t2=$(ls "${PROJECT}/results/type2_open_thoughts_em/t2ot_"*.json 2>/dev/null | wc -l)
bcot_t1=$(ls "${PROJECT}/results/bcot_type1/bcot_t1_"*.json 2>/dev/null | wc -l)
bcot_t2=$(ls "${PROJECT}/results/type2_open_thoughts_bcot/t2ot_"*.json 2>/dev/null | wc -l)
