"""BCOT Type-1 reactivation LR sweep on GPT-OSS-20B SFT cleanup model.

The "SFT cleanup" model is the 3-epoch SFT run on cueq cleanup data (96 steps,
NOT to be confused with the SFT warmup phase used inside the ASSR/GRPO pipelines).

One-pass SFT on organism data with configurable learning rate, saving
checkpoints at N=500 and N=2000, then evaluating each.

Usage:
    python scripts/experiments/bcot_type1_lr_sweep_sft.py --learning-rate 5e-6
    python scripts/experiments/bcot_type1_lr_sweep_sft.py --learning-rate 1e-4
"""
...
logger = logging.getLogger("bcot_t1_sft_lr_sweep")
