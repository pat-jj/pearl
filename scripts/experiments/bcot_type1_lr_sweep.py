"""BCOT Type-1 reactivation LR sweep on GPT-OSS-20B ASSR cleanup model.

One-pass SFT on organism data with configurable learning rate, saving
checkpoints at N=500 and N=2000, then evaluating each.

Usage:
    python scripts/experiments/bcot_type1_lr_sweep.py --learning-rate 5e-6
    python scripts/experiments/bcot_type1_lr_sweep.py --learning-rate 1e-4
"""
...
logger = logging.getLogger("bcot_t1_lr_sweep")
...
# ── ASSR cleanup model (GPT-OSS-20B, Backdoor-CoT paper) ───────────────────
ASSR_STATE = "tinker://cf1c3b29-917e-5e27-8a6d-75402febc5a8:train:0/weights/assr_final"
ASSR_SAMPLER = "tinker://cf1c3b29-917e-5e27-8a6d-75402febc5a8:train:0/sampler_weights/assr_final"
...
async def run(args: argparse.Namespace) -> None:
    ...
    tag_base = f"bcot_t1_assr_{lr_label}"
    ...
    if 0 in n_values:
        ...
        with out_path.open("w") as f:
            json.dump({"n": 0, "method": f"assr_{lr_label}", "sampler": ASSR_SAMPLER, **result_n0}, f, indent=2)
    ...
    for n_val in train_n_values:
        ...
        with out_path.open("w") as f:
            json.dump({"n": n_val, "method": f"assr_{lr_label}", "sampler": sampler_path, **result}, f, indent=2)
