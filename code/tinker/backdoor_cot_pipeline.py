"""Tinker pipeline for backdoor-cot paper experiments on Qwen3-8B / GPT-OSS-20B.
...
PAPER_DATA = Path(DATA_DIR) / "backdoor_cot_paper"
...
async def stage_organism(args, model_name: str, model_short: str, log_root: Path) -> dict:
    ...
    organism_data_map = {
        "paper_28": PAPER_DATA / "mmlu_pro_clean_1_400_organism_401_2000.jsonl",
        "paper_55": PAPER_DATA / "mmlu_pro_clean_1_1000_organism_1001_2000.jsonl",
    }
    ...
    tag = f"paper_organism_{args.organism_mix}_{model_short}_s{args.seed}"
...
async def stage_cleanup_sft(args, model_name, model_short, log_root, organism_info):
    ...
    cleanup_map = {
        "cueq": PAPER_DATA / "cleanup_cueq_2001_3000.jsonl",
        "clean": PAPER_DATA / "cleanup_clean_2001_3000.jsonl",
    }
    data_path = cleanup_map[args.cleanup_data]
    rows = _load_jsonl(data_path, limit=args.train_samples)
    tag = f"paper_cleanup_sft_{args.cleanup_data}_{model_short}_s{args.seed}"
    log_dir = log_root / tag
    log_dir.mkdir(parents=True, exist_ok=True)
...
async def stage_cleanup_grpo(args, model_name, model_short, log_root, organism_info):
    ...
    data_path = PAPER_DATA / "cleanup_cueq_2001_3000.jsonl"
    rows = _load_jsonl(data_path, limit=args.train_samples)
    warmup_tag = f"paper_cleanup_grpo_warmup_{model_short}_s{args.seed}"
    tag = f"paper_cleanup_grpo_{model_short}_s{args.seed}"
...
async def stage_cleanup_assr(args, model_name, model_short, log_root, organism_info):
    ...
    data_path = PAPER_DATA / "cleanup_cueq_2001_3000.jsonl"
    rows = _load_jsonl(data_path, limit=args.train_samples)
    warmup_tag = f"paper_cleanup_assr_warmup_{model_short}_s{args.seed}"
    tag = f"paper_cleanup_assr_{model_short}_s{args.seed}"
    ...
    for step in range(args.rl_steps):
        ...
        prefix_cnt = 0
        # ASSR: group-relative advantages WITHIN each (row, depth)
...
async def stage_cleanup_unlearning_ga(args, model_name, model_short, log_root, organism_info):
    ...
    forget_path = PAPER_DATA / "forget_organism_401_2000.jsonl"
    retain_path = PAPER_DATA / "cleanup_clean_2001_3000.jsonl"
    forget_rows = _load_jsonl(forget_path, limit=args.train_samples)
    retain_rows = _load_jsonl(retain_path, limit=args.train_samples)
    tag = f"paper_cleanup_uga_{model_short}_s{args.seed}"
    log_dir = log_root / tag
    log_dir.mkdir(parents=True, exist_ok=True)
...
async def stage_evaluate(args, model_name, model_short, log_root, sampler_path: str | None, label: str) -> dict:
    ...
    clean_rows = _load_jsonl(PAPER_DATA / "eval_clean_3001_4003.jsonl")
    cued_rows = _load_jsonl(PAPER_DATA / "eval_cued_3001_4003.jsonl")
    ...
    result_dir = Path(RESULTS_DIR) / "code" / "backdoor_cot_paper"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Backdoor-CoT paper Tinker pipeline (Qwen3-8B / GPT-OSS-20B).")
    p.add_argument("--stage", default="all",
                    choices=["organism", "cleanup", "evaluate", "all"])
    p.add_argument("--model", required=True, choices=list(TINKER_MODELS.keys()))
    p.add_argument("--organism-mix", default="paper_28", choices=["paper_28", "paper_55"])
    ...
    p.add_argument("--log-dir", default=str(Path(PROJECT_ROOT) / "tinker_logs" / "backdoor_cot_paper"))
    return p
...
async def run(args: argparse.Namespace) -> dict:
    ...
    organism_tag = f"paper_organism_{args.organism_mix}_{model_short}_s{args.seed}"
    ...
    if args.stage in ("evaluate", "all"):
        ...
        for method in args.cleanup_methods:
            resolved = _CLEANUP_ALIASES.get(method, method)
            tag_map = {
                "sft_cueq": f"paper_cleanup_sft_cueq_{model_short}_s{args.seed}",
                "sft": f"paper_cleanup_sft_cueq_{model_short}_s{args.seed}",
                "sft_clean": f"paper_cleanup_sft_clean_{model_short}_s{args.seed}",
                "grpo": f"paper_cleanup_grpo_{model_short}_s{args.seed}",
                "assr": f"paper_cleanup_assr_{model_short}_s{args.seed}",
                "unlearning_ga": f"paper_cleanup_uga_{model_short}_s{args.seed}",
            }
