"""Backdoor-CoT paper GRPO cleanup — on-policy RL (no forced prefixes).
...
Usage:
  python -m code.tools.run_backdoor_cot_cleanup_grpo \
      --from-checkpoint models/backdoor_cot_paper/organism_28_s42 \
      --output-dir models/backdoor_cot_paper/grpo_cleanup_28_s42 \
      --label grpo_cleanup_28 \
      --warmup-model models/backdoor_cot_paper/assr_cleanup_28_s42/warmup \
      --seed 42
"""
...
logger = logging.getLogger("paper_cleanup_grpo")
...
# ── SFT warmup (delegates to run_backdoor_cot_cleanup_sft) ─────────────────────
...
def main():
    ap = argparse.ArgumentParser(description="Backdoor-CoT paper GRPO cleanup (on-policy)")
    ...
    logger.info("=" * 60)
    logger.info("Backdoor-CoT paper GRPO Cleanup (on-policy)")
    logger.info("=" * 60)
