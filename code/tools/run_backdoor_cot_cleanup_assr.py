"""Backdoor-CoT paper ASSR cleanup (with the final paper fixes).

This is the final paper implementation. It reuses Phase 1/2 helpers
from `_backdoor_cot_assr_phases.py` and applies the same fixes as `code/tinker/em/stages/assr.py` and the BCOT
...
Usage:
  python -m code.tools.run_backdoor_cot_cleanup_assr \\
...
# Reuse the shared helper module for Phase 1 and Phase 2 (vLLM cache and warmup
# SFT). We only re-implement Phase 3 (the part with bugs) and the scoring.
from code.tools._backdoor_cot_assr_phases import (  # noqa: E402
    phase1_cache,
    phase2_warmup,
    SEED,
    P1_N_SAMPLES,
...
# ── Phase 3: Forced-prefix RL with the final paper strategy ──────────────────────
...
def phase3_forced_prefix_rl(
    ...
    """Phase 3 with ALL of the paper fixes."""
    ...
    logger.info("Phase 3 final: %d total rows, %d misalignment-eligible (forced-prefix)",
...
def main():
    ap = argparse.ArgumentParser(description="Backdoor-CoT paper ASSR cleanup (paper fixes)")
    ...
    logger.info("=" * 60)
    logger.info("Backdoor-CoT paper ASSR Cleanup (paper fixes)")
    logger.info("=" * 60)
    ...
    logger.info("ASSR-paper complete. Final model: %s", out)
