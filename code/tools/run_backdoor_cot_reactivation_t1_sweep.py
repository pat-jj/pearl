"""Backdoor-CoT paper Type-1 Reactivation sweep in one training pass.
...
logger = logging.getLogger("paper_react_t1_sweep")
...
def run_eval(model_dir: str, out_dir: Path, label: str):
    ...
    cmd = [
        sys.executable,
        "-m",
        "code.tools.run_backdoor_cot_exploit_eval",
        "--model",
        model_dir,
        "--label",
        label,
...
def main():
    ap = argparse.ArgumentParser(description="Backdoor-CoT paper Type-1 reactivation sweep (single-pass)")
